...
This is a private Preview verification workflow. Routine staff review was rejected as the launch strategy on September 18; these controls remain in place while the automatic product path is unqualified. It does not establish automatic visual judgment, a 90-second delivery promise, or production launch readiness. The compact reviewer study remains closed; none of its candidates become human approvals.
...
9. If paid generation is separately authorized and enabled, staff must explicitly select **Generate one candidate (paid)** again. The full saved brief, latest correction and prior notes accompany the latest rejected artwork through the image-edit endpoint. The original PNG is preferred only after its delivery transform matches the inspected image hash. An older record without an original can use its inspected PNG. Missing, corrupt or mismatched pixels block before dispatch; there is no text-only fallback. The input source hash, inspected image hash and candidate version are retained with the generation. This is a generative edit, so inspect for unintended changes as well as the requested correction. Prior rejected images remain available privately for comparison. Approval checks, note and full-size inspection reset for the new candidate. An archived image cannot be approved or delivered.

/** Pending v2 research scheduler. Owner-private Preview only; no customer activation.
...
import { prepareSeparatedReview, combineSeparatedReview, type SeparatedReviewReceipt, validateEvidenceReview } from "./aiFirst/separatedArtworkReview";
import { VISION_MODEL } from "./aiFirst/visionGate";
import { reviewProviderError } from "./aiFirst/reviewProviderError";
import proposedRegistration from "./evidenceReviewRegistration.json";

export type EvidenceReviewRegistration = Omit<typeof proposedRegistration, "authorizationStatus"> & { authorizationStatus: "pending" | "approved" };
...
export async function runEvidenceReviewStudy(event: Event, store: AiFirstArtworkAttemptStore, requestId: string, options: Options,
  registration: EvidenceReviewRegistration = proposedRegistration as EvidenceReviewRegistration) {
  ...
  if (registration.protocol !== "evidence-owned-research-v1" || registration.maximumCalls !== 20 ||
    ...
    registration.dataset !== "evidence-review-20260917-v1" || !Number.isSafeInteger(registration.budgetMicros) ||
    registration.budgetMicros <= 0 || registration.budgetMicros > 2_500_000 ||
    registration.reusedCraft.length !== 0 || registration.pricing.inputMicrosPerToken !== 3 || registration.pricing.outputMicrosPerToken !== 15 ||
    registration.cases.length !== 12 || new Set(registration.cases.map(c => c.caseId)).size !== 12 ||
    ...
    SEPARATED_STUDY_REQUESTS.some(r =>
      !["craft", "fidelity"].includes(r.role) || r.version !== "evidence-owned-decisions-v2" || r.planningInputTokens !== 15000 ||
      r.requestPlanningReserveMicros !== 45000 + 15 * r.maxOutputTokens ||
  ...
  const component = validateEvidenceReview(report, packet.context);
  ...
  if (r.role === "fidelity") {
    ...
    const craft = priorCraft?.componentContractValid === true ? priorCraft.receipt as SeparatedReviewReceipt : null;
  ...
  if (!continuationAllowed || index === SEPARATED_STUDY_REQUESTS.length - 1) {
    proof.closeReason = continuationAllowed ? "all-twenty-completed" : !accountingKnown ? "accounting-unknown" : "request-or-budget-failure";

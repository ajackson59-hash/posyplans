...
- CRITICAL — SUBJECT SELECTION: First classify the theme, then choose subjects accordingly. Read each lane's "Subject guidance" carefully for how to render, but WHAT to render depends on theme type:
  ...
  NAMED CHARACTER THEMES: When the host explicitly names a film, franchise or character theme, retain that exact named identity and every specifically requested character. The host determines cast size, setting, activities and props. Recognizing a property does not require a whole cast or an unrequested scene. A single-character portrait must remain a single-character portrait. Preserve recognizable identity rather than a generic adjacent aesthetic. Never add a franchise logo, rendered title, watermark or copied invitation layout.

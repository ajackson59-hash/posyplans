...
import registration from "../server/evidenceReviewRegistration.json";
vi.mock("../server/storage", () => ({ storage: {} }));
vi.mock("../server/db", () => ({ db: {} }));
vi.mock("../server/evidenceReviewStudy", () => ({ runEvidenceReviewStudy: vi.fn() }));
import { runEvidenceReviewStudy } from "../server/evidenceReviewStudy";
import { registerPrePaymentPreviewQualityRoutes } from "../server/prePaymentPreviewQualityRoutes";
const run = vi.mocked(runEvidenceReviewStudy);
const path = "/api/events/owner/offline-owner/prepayment-preview/evidence-review/craft-elsa";
...
it.each(["production", "wrong-branch", "wrong-owner", "wrong-event", "unknown-request"])("hides route for %s", async mode => {
  ...
  const url = mode === "wrong-owner" ? path.replace("offline-owner", "unknown") : mode === "unknown-request" ? path.replace("craft-elsa", "unknown") : path;

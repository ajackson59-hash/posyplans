...
// generation before checkout, but capped to a few attempts, gated behind a
// captured email, and served through a blur-only endpoint that never
// exposes the unblurred image before payment. Mirrors the supertest
...
describe("POST /prepayment-preview", () => {
  it("refuses an unpaid, un-emailed event before calling any model", async () => {
  ...
  it("generates and persists a preview once an email is captured", async () => {
  ...
  it("is idempotent: a second call does not spend again", async () => {
    stored.capturedEmail = "host@example.com";
    const app = await makeApp();
    await request(app).post(`/api/events/owner/${OWNER}/prepayment-preview`).send({});
    generateInviteIllustrationWithQualityGate.mockClear();
...
describe("GET /prepayment-preview/asset", () => {
  ...
  it("serves a downsampled, blurred PNG to an unpaid caller — never the raw bytes", async () => {

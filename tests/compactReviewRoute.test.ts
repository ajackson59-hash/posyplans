...
import registration from "../server/compactReviewRegistration.json";
vi.mock("../server/storage", () => ({ storage: {} }));
vi.mock("../server/db", () => ({ db: {} }));
vi.mock("../server/compactReviewStudy", () => ({ runCompactReviewStudy: vi.fn() }));
import { runCompactReviewStudy } from "../server/compactReviewStudy";
import { registerPrePaymentPreviewQualityRoutes } from "../server/prePaymentPreviewQualityRoutes";
const run = vi.mocked(runCompactReviewStudy);
const path = "/api/events/owner/offline-owner/prepayment-preview/compact-review/fidelity-c01";
...
it.each(["production", "wrong-branch", "wrong-owner", "wrong-event", "unknown-request"])("hides route for %s", async mode => {
  ...
  const url = mode === "wrong-owner" ? path.replace("offline-owner", "unknown") : mode === "unknown-request" ? path.replace("fidelity-c01", "unknown") : path;
...
it("allows retained candidate lookup without uploading its bytes", async () => {
  const { candidateBase64: _, ...input } = body;
  expect((await request(app()).post(path).send(input)).status).toBe(200);
  expect(run.mock.calls[0][3].candidate).toBeUndefined();
});
it("rejects noncanonical reference encoding", async () => {
  expect((await request(app()).post(path).send({ ...body, referenceBase64: ["YQ== "] })).status).toBe(400);

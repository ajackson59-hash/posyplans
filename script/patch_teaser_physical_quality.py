...
replace_once(
  ...
  const prompt = [
    ...
    "DEPTH/MATERIAL: directional key + subtle rim light; crisp hero plane, natural depth falloff; coherent shadows/color bounce. No waxy skin, plastic food, stamped bubbles, tiled clusters or cutout halos.",
    "HANDS/PROPS: simple natural hands; unless required, no food/small props in hands—put treats on a stable surface at believable scale.",
    "COMPOSITION: fully frame faces/hands/required objects; avoid dense repeated micro-objects as foreground hero; control saturation/clutter.",
    milestoneDirection,
...
replace_once(
    ...
    expect(concept.art.prompt).toContain("no food/small props in hands");
    expect(concept.art.prompt).toContain("MILESTONE:");''',
)
